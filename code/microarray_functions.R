...
run_pseudobulk_dea <- function(
  ...
  missing_metadata <- setdiff(sample_ids, rownames(metadata))
  metadata <- metadata[sample_ids, , drop=F]
  missing_covariates <- setdiff(covariate_cols %||% character(0), colnames(metadata))
  group_values <- as.character(metadata[[group_col]])
  if (is.null(group_levels)) group_levels <- unique(group_values)
  group <- factor(group_values, levels = group_levels)
  ...
  design_formula <- if (is.null(covariate_cols) || !length(covariate_cols)) {
    ...
    stats::as.formula(paste0("~ 0 + group + ", paste(covariate_cols, collapse = " + ")))
  }
  design <- stats::model.matrix(design_formula, data = design_data)
  colnames(design) <- sub("^group", "", colnames(design))
  colnames(design) <- make.names(colnames(design))

  y <- edgeR::DGEList(counts = round(counts), samples = metadata)
  keep_genes <- edgeR::filterByExpr(y, design=design)
  y <- y[keep_genes, , keep.lib.sizes = F]
  y <- edgeR::calcNormFactors(y, method = "TMM")
  v <- limma::voom(y, design = design, plot = F)
  fit <- limma::lmFit(object = v, design = design)

  if (is.null(contrast_str)) {
    if (nlevels(group) != 2L) stop("Automatic contrast generation requires exactly two levels. Provide 'contrast_str' for >2 groups.")
    lev <- make.names(levels(group))
    contrast_str <- paste0(lev[2L], "-", lev[1L])
  }
  contrast_matrix <- limma::makeContrasts(contrasts = contrast_str, levels = design)
  fit2 <- limma::contrasts.fit(fit, contrasts = contrast_matrix)
  fit2 <- limma::eBayes(fit2, robust = robust)
  results <- limma::topTable(
    fit2,
    coef = 1L,
    number = Inf,
  ...
  if (save.view) {
    if (!is.null(save.dir)) stop("'save.dir' must be provided.")
    dir.create(save.dir, recursive = T, showWarnings = F)
    save_prefix <- gsub(pattern = "[^A-Za-z0-9_.-]", replacement = "_", x = output_prefix)
    output_file <- file.path(save.dir, paste0(save_prefix, "_dea.csv"))

    utils::write.csv(results file = output_file, row.names = F, quote = F)
  }
  list(dge = y, voom = v, fit = fit2, design = design, metadata = metadata, contrast = contrast_str,
  n_samples_by_group = n_samples_by_group, genes_tested = rownames(y), results = results)
}
